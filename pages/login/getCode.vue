<template>
	<view>
		...
		<view class="btn-wrap">
			<!-- #ifdef APP-PLUS -->
			<m-button title="获取验证码" :css="{'border-radius': '40px'}"
				:style="{'background':isPhone?'#ffc576':'#ff6a00' }" :disabled="isPhone" @handleClick="handleLogin">
			</m-button>
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<button style="border-radius: 40px;background:#ff6a00;color:#FFFFFF" open-type="getPhoneNumber"
				@getphonenumber="getPhoneNumberClick">手机号码一键登录</button>
			<!-- #endif -->
		</view>
		...
		<view class="login-foot">
				...
				<view class="wx-btn-v">
					<view class="wx-btn-box">
						<view class="">
							...
							<!-- #ifdef MP-WEIXIN -->
							<button type="primary" class="wx-btn-b" open-type="getUserInfo" lang="zh_CN"
								@getuserinfo="appLoginWx"></button>
							<!-- #endif -->
							<!-- #ifdef APP-PLUS -->
						...
						<view class="c-apple-login-btn" v-if="system" @click="appleAuth">
							<image style="width: 300rpx; background-color: #eeeeee;margin: 30rpx 0 0 40rpx"
								src="/static/images/appleLogin.png" mode="widthFix"></image>
						</view>
					</view>
				</view>
			...
			<view class="tip-wrap">
				...
				<u-checkbox-group>
					<u-checkbox v-model="agreeFlag" name="1" inactive-color="#eeeeee" active-color="#ff5000"
						label-size="24rpx">
						请阅读并同意
					</u-checkbox>
				</u-checkbox-group>
		...
		methods: {
			...
			async handleLogin() {
				...
				const {
					statusCode,
					data
				} = await this.$u.api.sendLoginCode(params, true, {
			...
			async appLoginFun(newParams) {
				...
				const {
					...
					data,
					message
				} = await that.$u.api.wechat(newParams)
			...
			async loginFunWx(params) {
				...
				const {
					...
					data,
					message
				} = await this.$u.api.wechatPhone(params)
				if (statusCode === 200 && data.error == 0) {
					this.$toast('登录成功')
					const token = data.token
					console.log(token)
			...
			async loginFun(params, type) {
				const {
					...
					data,
					message
				} = this.system && type ? await this.$u.api.appleLogin(params) : await this.$u.api.miniWechat(params)
				if (statusCode === 200 && data.error == 0) {
					this.$toast('登录成功')
					const token = data.token
					console.log(token)
			...
			navToFuc(type) {
				if (type == 1) {
					navTo(
						'/publicPages/pages/news/webView?url=https://app.xiaogebang.com/index.php/Api/Article/detail/id/81')
				} else {
					navTo('')
				}
			},
