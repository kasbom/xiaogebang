      ...
      <swiper-item v-for="(tab, indexs) in orderList" :key="indexs" class="swiper-item">
        <scroll-view class="scroll-v list" enableBackToTop="true" scroll-y @scrolltolower="loadMore(indexs)">
						<view class="order-wrap">
							<OrderList :orderList="tab.data" @getList="reGetList" />
						</view>
  ...
  import config from '@/config/index.js'
	...
	export default {
      ...
      return {
		...
		onLoad(option) {
			this.activeIndex = (Number(option.status)-1)||''
    ...
    methods: {
		   reGetList(){
			...
			async getList () {
				...
				// tab.data=[]
				let newStatus=tab?tab.id:''
				const page_size = 10
				const { statusCode, data } = await this.$u.api.getOrderList({ status:newStatus, page: tab.page ,page_size})
